...
void evasiveManeuvers(int Xdest, int Ydest){
	...
	/*
	cout << "Protocol: ";
	switch(RobotPosition.getAng()/90){
		...
		*/
	cout << "I'm at (" <<RobotPosition.curPos.EW/TICKSPERFOOT <<"," <<RobotPosition.curPos.NS/TICKSPERFOOT << ")" << endl; 
	if(abs(RobotPosition.curPos.NS/TICKSPERFOOT) > abs(RobotPosition.curPos.EW/TICKSPERFOOT)){
		if(RobotPosition.curPos.NS/TICKSPERFOOT > 0){
			cout << "Protocol South" << endl;
			turnToFace(2);
		}else{
			cout << "Protocol North" << endl;
			turnToFace(0);
		}
	}else{
		if(RobotPosition.curPos.EW/TICKSPERFOOT >0){
			cout << "Protocol West" << endl;
			turnToFace(3);
		}else{
			cout << "Protocol East" << endl;
			turnToFace(1);
		}
	}
	goForward(2*TICKSPERFOOT);
...
string findBlock(int fd){
	...
	string num = camGetLineObjDetect(fd);
	camStreamOff(fd);
	if(num == "error"){
		findFailures++;
		switch(findFailures){
			case 1:
				cout << "Find Block First Failure" << endl;
				goBackward(.5*TICKSPERFOOT,RobotPosition.getAng());
				num = findBlock(fd);
				cout << "First Failure Return " << num << endl;
				goForward(.5*TICKSPERFOOT);
				break;
			case 2:
				cout << "Find Block Secound Failure" << endl;
				pivotLeft(1,15);
				num = findBlock(fd);
				cout << "Secound Failure Return " << num << endl;
				pivotRight(1,15);
				break;
			case 3:
				cout << "Find Block Third Failure" << endl;
				pivotRight(1,30);
				num = findBlock(fd);
				cout << "Third Failure Return " << num << endl;
				pivotLeft(1,30);
				break;
			case 4:
				cout << "Find Block Fourth Failure" << endl;
				pivotLeft(1,15);
				goForward(1.5*TICKSPERFOOT);
				findFailures = 0;
				cout << "You're stuck in a deep, dark hole of recursion" << endl;
				num = findBlock(fd);
				cout << "Fourth Failure Return " << num << endl;
				goBackward(1.5*TICKSPERFOOT,RobotPosition.getAng());
				break;
			default:
				cout << "Find Block Unknown Failure" << endl;
		}		
	}
...
void findBlockInSquare(){
	...
	do{
		...
		halt();
		findFailures = 0;
		diff = atoi((findBlock(fdJevois)).c_str());
...
void punchersDown(){
	if(RobotPosition.getPuncher() == 1 || true){
...
void punchersUp(){
	if(RobotPosition.getPuncher() == 0||true){
		printf("Punchers Up\n");
...
char examineMothership(){
	...
	do{
		// Note to Adam: Do not change these signs -Adam
		if (diff < 0){
			cout << "Strafe Left to Center " << -diff*1 << endl;
			strafeLeft((int)(-diff*1));
		} else if ( diff > 0)  { //I didn't listen -Adam
			cout << "Strafe Right to Center " << diff*1 << endl;
			strafeRight((int)(diff*1));
		} 
		halt();
		activateObjectDetect(fdJevois);
		str = camGetLineObjDetect(fdJevois);
		str = str.substr(str.find("jpg")+4,4);
...
// Dillon 3/19
int lookForBlock(int blockX, int blockY){
	cout << "Look for block in (" << blockX/TICKSPERFOOT << "," << blockY/TICKSPERFOOT << ")" << endl;
	//activateObjectDetect(fdJevois);
...
double MSstartX,MSstartY;

int lookForMothership(){
	int index;
	std::pair <double,double> corner0 (1.5,1.5);
	std::pair <double,double> corner1 (1.5,-2.5);
	std::pair <double,double> corner2 (-2.5,-2.5);
	std::pair <double,double> corner3 (-2.5,1.5);
	
	pair<double,double> corners[] = {corner0,corner1,corner2,corner3};
	pair<double,double> start = chooseDest(corners);
	start.first = MSstartX;
	start.second =  MSstartY;
	cout << "start X " << MSstartX << endl;
	cout << "start Y " << MSstartY << endl;
	if(MSstartX > 0 && MSstartY > 0)
		index = 0;
	else if(MSstartX > 0 && MSstartY < 0)
		index = 1;
	else if(MSstartX < 0 && MSstartY < 0)
		index = 2;
	else
		index = 3;
	corners[index].first = MSstartX;
	corners[index].second = MSstartY;
	while(index<4)
	{	
		cout << "starting at corner " << index << endl;
		cout << "corner X " << corners[index].first << endl;
		cout << "corner Y " << corners[index].second << endl;
		goToPointNS(corners[index].first*TICKSPERFOOT,corners[index].second*TICKSPERFOOT);
		++index%4;
		if(msFound)
		{
			cout << "go to Mother ship" << endl;
			goToPointNS(motherX,motherY);
			examineMothership();
			return 0;
		}
	}
...
pair<double,double> chooseDest(pair<double,double> coordinates[]){
	...
	MSstartX = coordinates[index].first;
	MSstartY = coordinates[index].second;
...
pair<double,double>* readJson(){
	...
	//static pair<double,double> locals[6];
